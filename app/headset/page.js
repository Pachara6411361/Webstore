...
import Headset from "../public/headgear.jpg";
...
const HeadsetPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get("/api/products?category=headset");
        ...
        {products.map((product, index) => (
          <div key={index} className="product-card">
            {product.image ? (
              ...
              <Image src={Headset} alt={product.name} width={150} height={300} />
            )}
